// جلب العملات من Raydium وتطبيق الاستراتيجية
const fetch = (...args) => import('node-fetch').then(({default: fetch}) => fetch(...args));
const strategy = { minVolume: 7, minHolders: 34, minAge: 3, enabled: true };
async function main() {
  console.log('مصدر Raydium:');
  try {
    const res = await fetch('https://api.raydium.io/v2/sdk/token/list');
    const data = await res.json();
    const tokens = Array.isArray(data?.official) ? data.official : [];
