...
var PinBuilder = class {
  ...
  passive(name) {
    ...
    const centerX = this.x + Math.sign(this.lastDx) * (Math.abs(this.lastDx) > 0 ? halfWidth : 0);
    const centerY = this.y + Math.sign(this.lastDy) * (Math.abs(this.lastDy) > 0 ? halfHeight : 0);
...
var convertNormalizedNetlistToInputNetlist = (normalizedNetlist) => {
  const boxes2 = normalizedNetlist.boxes.map((b) => ({
  ...
  for (const targetConnection of targetConnections) {
    ...
    const connectionExists = templateConnections.some((templateConnection) => {
      ...
      const hasFromChipPin = templateBoxPorts.some(
        (port) => port.boxId === templateFromChipId && port.pinNumber === fromPort.pinNumber
      );
      const hasToChipPin = templateBoxPorts.some(
        (port) => port.boxId === templateToChipId && port.pinNumber === toPort.pinNumber
      );
...
var applyCircuitLayoutToCircuitJson = (circuitJson, circuitJsonNetlist, layout) => {
  ...
  const netIndexToCompositeNetId = /* @__PURE__ */ new Map();
  ...
  for (const layoutLabel of layout.netLabels) {
    ...
    const compositeNetId = netIndexToCompositeNetId.get(netIndex) ?? "ERROR: did not find netId using net index";
    const newLabel = {
      type: "schematic_net_label",
      schematic_net_label_id: compositeNetId,
      source_net_id: layoutLabel.netId,
      // Assumes layoutLabel.labelId is the source_net identifier
      text: compositeNetId.split(",").find((n) => !n.includes(".")) ?? compositeNetId,
...
function circuitBuilderFromLayoutJson(layout) {
  ...
  for (const path of layout.paths) {
    ...
    C.lines.push({
      start: { x: start.x, y: start.y, ref: path.from },
      end: { x: end.x, y: end.y, ref: path.to },
