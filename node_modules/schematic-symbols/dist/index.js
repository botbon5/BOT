...
var battery_vert_default = rotateRightFacingSymbol(battery_horz_default, {
  newOrientation: "up"
});
...
var { 6: letter, 2: underline, ...rest } = dc_ammeter_horz_default.primitives;
...
// symbols/solderjumper2_bridged12_down.ts
var solderjumper2_bridged12_down_default = modifySymbol(solderjumper2_bridged12_default).rotateRightFacingSymbol("down").labelPort("left1", ["1"]).labelPort("right1", ["2"]).changeTextAnchor("{REF}", "middle_left").build();

// symbols/solderjumper2_bridged12_left.ts
var solderjumper2_bridged12_left_default = modifySymbol(solderjumper2_bridged12_default).rotateRightFacingSymbol("left").labelPort("left1", ["1"]).labelPort("right1", ["2"]).changeTextAnchor("{REF}", "middle_top").build();

// symbols/solderjumper2_bridged12_right.ts
var solderjumper2_bridged12_right_default = modifySymbol(solderjumper2_bridged12_default).rotateRightFacingSymbol("right").labelPort("left1", ["1"]).labelPort("right1", ["2"]).changeTextAnchor("{REF}", "middle_bottom").build();

// symbols/solderjumper2_bridged12_up.ts
var solderjumper2_bridged12_up_default = modifySymbol(solderjumper2_bridged12_default).rotateRightFacingSymbol("up").labelPort("left1", ["1"]).labelPort("right1", ["2"]).changeTextAnchor("{REF}", "middle_right").build();

// assets/generated/solderjumper2.json
var solderjumper2_default = {
  paths: {
    path44: {
      type: "path",
      points: [
...
var tilted_ground_default = {
  paths: {
    ...
    path45: {
      ...
      points: [
        {
          x: -0.4021990999999996,
          y: 0.20403629999999837
        },
        {
          x: -0.108693999999996,
          y: 0.04403629999999836
...
// symbols/tilted_ground_right.ts
...
// symbols/usbc.ts
var usbc_default2 = modifySymbol(usbc_default).changeTextAnchor("{REF}", "middle_bottom").build();
...
var symbols_index_default = {
  ...
  "potentiometer3_up": potentiometer3_up_default,
  ...
  "solderjumper2_bridged12_left": solderjumper2_bridged12_left_default,
  "solderjumper2_bridged12_right": solderjumper2_bridged12_right_default,
  "solderjumper2_bridged12_up": solderjumper2_bridged12_up_default,
  "solderjumper2_down": solderjumper2_down_default,
  ...
  "tilted_ground_up": tilted_ground_up_default,
  ...
  "unijunction_transistor_horz": unijunction_transistor_horz_default,
  "unijunction_transistor_vert": unijunction_transistor_vert_default,
  "usbc": usbc_default2,
...
// drawing/utils/makeYUpPositive.ts
var makeYUpPositive = (y, yUpPositive = true) => {
...
function pathToSvgD(points, {
  ...
  const pathCommands = points.map(
    (point, index) => `${index === 0 ? "M" : "L"}${point.x},${makeYUpPositive(point.y, yUpPositive)}`
  ).join(" ");
  return closed ? `${pathCommands} Z` : pathCommands;
}

// drawing/getSvg.ts
function createDiamondElement(center, size = 0.05, yUpPositive = true) {
  const { x, y } = center;
  const halfSize = size / 2;
  return `<path d="M ${x} ${makeYUpPositive(y, yUpPositive) - halfSize} L ${x + halfSize} ${makeYUpPositive(y, yUpPositive)} L ${x} ${makeYUpPositive(y, yUpPositive) + halfSize} L ${x - halfSize} ${makeYUpPositive(y, yUpPositive)} Z" fill="green" />`;
}
function createTextElement(primitive, { yUpPositive }) {
  ...
  return {
    text: `<text x="${x}" y="${makeYUpPositive(y, yUpPositive)}" dx="${dx}" dy="${dy}" text-anchor="${textAnchor}" style="font: ${fontSize ?? 0.1}px monospace; fill: ${mapColor("primary")}">${text}</text>`,
    anchor: `<rect x="${x - 0.025 / 2}" y="${makeYUpPositive(y, yUpPositive) - 0.025 / 2}" width="0.025" height="0.025" fill="blue" />`
  };
}
function createPortElement(port, { yUpPositive }) {
  const { x, y, labels } = port;
  const rectSize = 0.05;
  const labelFontSize = 0.08;
  const label = labels[0] || "";
  return `
    <rect x="${x - rectSize / 2}" y="${makeYUpPositive(y, yUpPositive) - rectSize / 2}" width="${rectSize}" height="${rectSize}" fill="red" />
    ...
    switch (primitive.type) {
      case "path":
        return `<path d="${pathToSvgD(primitive.points, {
          ...
          yUpPositive: true
        })}" fill="${primitive.fill ? mapColor(primitive.color) : "none"}" stroke="${mapColor(
          primitive.color
      ...
      case "circle":
        return `<circle cx="${primitive.x}" cy="${makeYUpPositive(primitive.y, true)}" r="${primitive.radius}" fill="${primitive.fill ? mapColor(primitive.color) : "none"}" ${!primitive.fill ? `stroke="${mapColor(primitive.color)}" stroke-width="0.02"` : ""} />`;
      case "box":
        return `<rect x="${primitive.x}" y="${makeYUpPositive(primitive.y)}" width="${primitive.width}" height="${primitive.height}" fill="${mapColor("primary")}" />`;
      default:
        return "";
    }
  });
