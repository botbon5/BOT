...
function getCCTPBridgePDAs(mint, destinationChain) {
  const cctpCoreProgramId = new import_web3.PublicKey(addresses_default.CCTP_CORE_PROGRAM_ID);
  const cctpTokenProgramId = new import_web3.PublicKey(addresses_default.CCTP_TOKEN_PROGRAM_ID);
  const [messageTransmitter] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("message_transmitter")],
    cctpCoreProgramId
  );
  const [senderAuthority] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("sender_authority")],
    cctpTokenProgramId
  );
  const [localToken] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("local_token"), mint.toBytes()],
    cctpTokenProgramId
  );
  const [tokenMessenger] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("token_messenger")],
    cctpTokenProgramId
  );
  const [tokenMinter] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("token_minter")],
    cctpTokenProgramId
  );
  const destinationDomain = getCCTPDomain(destinationChain);
  const [remoteTokenMessengerKey] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("remote_token_messenger"), import_buffer.Buffer.from(destinationDomain.toString())],
    cctpTokenProgramId
  );
  const [eventAuthCore] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("__event_authority")],
    cctpCoreProgramId
  );
  const [eventAuthToken] = import_web3.PublicKey.findProgramAddressSync(
    [import_buffer.Buffer.from("__event_authority")],
    cctpTokenProgramId
  );
  return {
...
function nativeAddressToHexString(address, wChainId) {
  if (wChainId === 1) {
    return (0, import_ethers.zeroPadValue)(new import_web32.PublicKey(address).toBytes(), 32);
  } else if (wChainId === chains.ethereum || wChainId === chains.bsc || wChainId === chains.polygon || wChainId === chains.avalanche || wChainId === chains.arbitrum || wChainId === chains.optimism || wChainId === chains.base || wChainId === chains.unichain || wChainId === chains.linea || wChainId === chains.sonic) {
    ...
    if (import_buffer2.Buffer.from(addressStr, "hex").length !== 32) {
...
function getAssociatedTokenAddress(mint, owner, allowOwnerOffCurve = false, programId = new import_web32.PublicKey(addresses_default.TOKEN_PROGRAM_ID), associatedTokenProgramId = new import_web32.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID)) {
  if (!allowOwnerOffCurve && !import_web32.PublicKey.isOnCurve(owner.toBuffer())) {
    throw new Error("TokenOwnerOffCurveError");
  }
  const [address] = import_web32.PublicKey.findProgramAddressSync(
...
function getSafeU64Blob(value) {
  ...
  const buf = import_buffer2.Buffer.alloc(8);
...
var ZeroPermit = {
  ...
  deadline: 0,
  v: 0,
  r: `0x${import_web32.SystemProgram.programId.toBuffer().toString("hex")}`,
  s: `0x${import_web32.SystemProgram.programId.toBuffer().toString("hex")}`
};
function wait(time) {
...
function getHyperCoreUSDCDepositCustomPayload(quote, destinationAddress, usdcPermitSignature) {
  const payload = import_buffer2.Buffer.alloc(109);
  const destAddressBuf = import_buffer2.Buffer.from(hexToUint8Array(destinationAddress));
  if (destAddressBuf.length !== 20) {
    throw new Error("Invalid destination address length, expected 20 bytes");
  }
  const permitSignatureBuf = import_buffer2.Buffer.from(
...
function decodeInstructionArgs(idl, data) {
  const disc = data.subarray(0, 8);
  const ix = idl.instructions.find(
    (ix2) => import_buffer4.Buffer.from(ix2.discriminator).toString("hex").toLowerCase() === import_buffer4.Buffer.from(disc).toString("hex").toLowerCase()
  );
  if (!ix)
    throw new Error(`Instruction ${import_buffer4.Buffer.from(disc).toString("hex")} not in IDL`);
...
function decodeType(type, buffer, offset, idl) {
  if (typeof type === "string") {
    switch (type) {
      ...
      case "pubkey": {
        ...
        return [new import_web34.PublicKey(slice), offset + 32];
      }
      default:
  ...
  if ("defined" in type) {
    ...
    if (def.type.kind === "struct") {
      ...
      if (!variant)
        throw new Error(`Invalid enum index ${idx}`);
      let cur = offset + 1;
      const payload = { variant: variant.name };
...
function decodeJupiterV6InsArgs(args) {
  return decodeInstructionArgs(jupV6Idl, import_buffer4.Buffer.from(args));
}
var jupV6Idl = {
...
function getConnection(rpcUrl) {
  cachedConnections[rpcUrl] ??= new import_web35.Connection(rpcUrl);
  return new import_web35.Connection(rpcUrl);
}
...
function createAssociatedTokenAccountInstruction(payer, associatedToken, owner, mint, programId = new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID), associatedTokenProgramId = new import_web35.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID)) {
  const keys = [
...
async function createInitializeRandomTokenAccountInstructions(connection, payer, mint, owner, keyPair, programId = new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  const instructions = [];
  const rentLamports = await connection.getMinimumBalanceForRentExemption(TOKEN_ACCOUNT_LEN);
  instructions.push(import_web35.SystemProgram.createAccount({
    fromPubkey: payer,
  ...
  instructions.push(new import_web35.TransactionInstruction({
    keys: [
      { pubkey: keyPair.publicKey, isWritable: true, isSigner: false },
      { pubkey: mint, isWritable: false, isSigner: false },
      { pubkey: owner, isWritable: false, isSigner: false },
      { pubkey: import_web35.SYSVAR_RENT_PUBKEY, isWritable: false, isSigner: false }
    ],
    programId,
    data: import_buffer5.Buffer.from([1])
  }));
...
function createApproveInstruction(account, delegate, owner, amount, programId = new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  const keys = [
  ...
  return new import_web35.TransactionInstruction({ keys, programId, data });
}
...
function createSyncNativeInstruction(account) {
  ...
  return new import_web35.TransactionInstruction({
    keys,
    programId: new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID),
    data
  });
}
...
function createCloseAccountInstruction(account, destination, owner, programId = new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  const keys = [
    { pubkey: account, isSigner: false, isWritable: true },
...
function createSplTransferInstruction(source, destination, owner, amount, programId = new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) {
  const keys = [
...
var solMint = new import_web35.PublicKey("So11111111111111111111111111111111111111112");
async function wrapSol(owner, amount, signTransaction, connection) {
  const solanaConnection = connection ?? new import_web35.Connection("https://rpc.ankr.com/solana");
  const toAccount = getAssociatedTokenAddress(solMint, owner, false);
  const {
    blockhash,
    lastValidBlockHeight
  } = await solanaConnection.getLatestBlockhash();
  const trx = new import_web35.Transaction({
    feePayer: owner,
    blockhash,
    lastValidBlockHeight
  });
  ...
  trx.add(import_web35.SystemProgram.transfer({
    fromPubkey: owner,
    toPubkey: toAccount,
...
async function unwrapSol(owner, amount, signTransaction, connection) {
  ...
  const delegate = import_web35.Keypair.generate();
  const {
    blockhash,
    lastValidBlockHeight
  } = await solanaConnection.getLatestBlockhash();
...
function deserializeInstructionInfo(instruction) {
  return new import_web35.TransactionInstruction({
    programId: new import_web35.PublicKey(instruction.programId),
    keys: instruction.accounts.map((key) => ({
      pubkey: new import_web35.PublicKey(key.pubkey),
...
async function getAddressLookupTableAccounts(keys, connection) {
  const addressLookupTableAccountInfos = await connection.getMultipleAccountsInfo(
    keys.map((key) => new import_web35.PublicKey(key))
  );
  return addressLookupTableAccountInfos.reduce((acc, accountInfo, index) => {
    const addressLookupTableAddress = keys[index];
    if (accountInfo) {
      const addressLookupTableAccount = new import_web35.AddressLookupTableAccount({
        key: new import_web35.PublicKey(addressLookupTableAddress),
        state: import_web35.AddressLookupTableAccount.deserialize(accountInfo.data)
      });
...
function getAnchorInstructionData(name) {
  ...
  return import_buffer5.Buffer.from(import_js_sha256.sha256.digest(preimage)).subarray(0, 8);
}
async function decideRelayer() {
  let relayer;
  try {
    const suggestedRelayer = await getSuggestedRelayer();
    relayer = new import_web35.PublicKey(suggestedRelayer);
  } catch (err) {
    console.log("Relayer not found, using system program");
    relayer = import_web35.SystemProgram.programId;
  }
  return relayer;
}
function getJitoTipTransfer(swapper, blockhash, lastValidBlockHeight, options) {
  ...
  return new import_web35.Transaction({
    feePayer: new import_web35.PublicKey(swapper),
    blockhash,
    lastValidBlockHeight
  }).add(import_web35.SystemProgram.transfer({
    fromPubkey: new import_web35.PublicKey(swapper),
    toPubkey: new import_web35.PublicKey(jitoAccount),
...
function validateJupCleanupInstruction(instruction, validCleanReceiverAddress) {
  ...
  if (!instruction.programId.equals(new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) && !instruction.programId.equals(new import_web35.PublicKey(addresses_default.TOKEN_2022_PROGRAM_ID))) {
...
function validateJupSetupInstructions(instructions, owner) {
  ...
  instructions.forEach((instruction) => {
    if (!instruction.programId.equals(new import_web35.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID)) && !instruction.programId.equals(import_web35.SystemProgram.programId) && !instruction.programId.equals(new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID)) && !instruction.programId.equals(new import_web35.PublicKey(addresses_default.TOKEN_2022_PROGRAM_ID))) {
      throw new Error("Invalid setup instruction:: programId");
    }
    if (instruction.programId.equals(new import_web35.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID))) {
      if (Uint8Array.from(instruction.data).length === 1) {
    ...
    } else if (instruction.programId.equals(import_web35.SystemProgram.programId)) {
      if (!owner) {
...
function validateJupSwapInstruction(instruction, validDestAccount, sameSourceAndDestWallet = false) {
  const JUP_PROGRAM_ID = new import_web35.PublicKey("JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4");
...
function validateJupComputeBudgetInstructions(instructions) {
  instructions.forEach((instruction) => {
    if (!instruction.programId.equals(import_web35.ComputeBudgetProgram.programId)) {
...
function createTransferAllAndCloseInstruction(owner, mint, tokenAccount, transferDestination, closeDestination, tokenProgramId = new import_web35.PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")) {
  return new import_web35.TransactionInstruction({
    keys: [
      { pubkey: owner, isSigner: true, isWritable: false },
    ...
    programId: new import_web35.PublicKey("B96dV3Luxzo6SokJx3xt8i5y8Mb7HRR6Eec8hCjJDT69"),
...
function createPayloadWriterCreateInstruction(payer, payloadAccount, payload, nonce) {
  ...
  return new import_web35.TransactionInstruction({
    keys,
    programId: new import_web35.PublicKey(addresses_default.PAYLOAD_WRITER_PROGRAM_ID),
    data: insData
  });
}
...
function getWormholePDAs(supplierProgram) {
  ...
  const programId = new import_web36.PublicKey(supplierProgram);
  const [bridgeConfig] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer6.Buffer.from("Bridge")],
    wormholeProgramId
  );
  const [emitter] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer6.Buffer.from("emitter")],
    programId
  );
  const [sequenceKey] = import_web36.PublicKey.findProgramAddressSync(
    [import_buffer6.Buffer.from("Sequence"), emitter.toBuffer()],
    wormholeProgramId
  );
  const [feeCollector] = import_web36.PublicKey.findProgramAddressSync(
...
function createMctpBridgeWithFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  const wormholeProgramId = new import_web37.PublicKey(addresses_default.WORMHOLE_PROGRAM_ID);
  const TOKEN_PROGRAM_ID = new import_web37.PublicKey(addresses_default.TOKEN_PROGRAM_ID);
  const cctpCoreProgramId = new import_web37.PublicKey(addresses_default.CCTP_CORE_PROGRAM_ID);
  const cctpTokenProgramId = new import_web37.PublicKey(addresses_default.CCTP_TOKEN_PROGRAM_ID);
  const mctpProgram = new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const relayer = new import_web37.PublicKey(relayerAddress);
  const mint = new import_web37.PublicKey(mintAddress);
  ...
  if (feeSolana && feeSolana > BigInt(0)) {
    ...
    relayerAccount = new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  }
  const cctpBridgePdas = getCCTPBridgePDAs(mint, toChain);
  const wormholePDAs = getWormholePDAs(addresses_default.MCTP_PROGRAM_ID);
  const cctpMessage = import_web37.Keypair.generate();
  const wormholeMessage = import_web37.Keypair.generate();
  const accounts = [
    ...
    { pubkey: import_web37.SYSVAR_CLOCK_PUBKEY, isWritable: false, isSigner: false },
    { pubkey: import_web37.SYSVAR_RENT_PUBKEY, isWritable: false, isSigner: false },
  ...
  const bridgeIns = new import_web37.TransactionInstruction({
    keys: accounts,
    data,
...
function createMctpBridgeLockFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  const instructions = [];
  const TOKEN_PROGRAM_ID = new import_web37.PublicKey(addresses_default.TOKEN_PROGRAM_ID);
  const ASSOCIATED_TOKEN_PROGRAM_ID = new import_web37.PublicKey(addresses_default.ASSOCIATED_TOKEN_PROGRAM_ID);
  const cctpCoreProgramId = new import_web37.PublicKey(addresses_default.CCTP_CORE_PROGRAM_ID);
  const cctpTokenProgramId = new import_web37.PublicKey(addresses_default.CCTP_TOKEN_PROGRAM_ID);
  const mctpProgram = new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const relayer = new import_web37.PublicKey(relayerAddress);
  ...
  const cctpBridgePdas = getCCTPBridgePDAs(mint, toChain);
  const cctpMessage = import_web37.Keypair.generate();
  const [feeState] = import_web37.PublicKey.findProgramAddressSync(
...
function createMctpInitSwapInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const cctpBridgePdas = getCCTPBridgePDAs(mint, toChain);
  const cctpMessage = import_web37.Keypair.generate();
  const [swapState] = import_web37.PublicKey.findProgramAddressSync(
  ...
  const initSwapIns = new import_web37.TransactionInstruction({
    keys: accounts,
    data,
    programId: mctpProgram
  });
...
function createMctpBridgeLedgerInstruction(params) {
  ...
  const user = new import_web37.PublicKey(params.swapperAddress);
  const mint = new import_web37.PublicKey(params.mintAddress);
  ...
  const accounts = [
    ...
    { pubkey: params.customPayload || new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID), isWritable: false, isSigner: false },
    { pubkey: mint, isWritable: false, isSigner: false },
    { pubkey: import_web37.SystemProgram.programId, isWritable: false, isSigner: false },
    { pubkey: new import_web37.PublicKey(refAddress), isWritable: false, isSigner: false }
  ];
  ...
  return new import_web37.TransactionInstruction({
    keys: accounts,
    data,
    programId: new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID)
  });
}
...
async function createMctpFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const mctpProgram = new import_web37.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const user = new import_web37.PublicKey(swapperAddress);
  ...
  const [ledger] = import_web37.PublicKey.findProgramAddressSync(
    [import_buffer7.Buffer.from(ledgerSeedPrefix), user.toBytes(), randomKey.publicKey.toBytes()],
    mctpProgram
  );
  const ledgerAccount = getAssociatedTokenAddress(
    new import_web37.PublicKey(quote.mctpInputContract),
    ledger,
    true
  );
  ...
  if (quote.fromToken.contract === quote.mctpInputContract) {
    ...
    instructions.push(
      createAssociatedTokenAccountInstruction(user, ledgerAccount, ledger, new import_web37.PublicKey(quote.mctpInputContract))
    );
    instructions.push(
...
function createSwiftOrderHash(quote, swapperAddress, destinationAddress, referrerAddress, randomKeyHex) {
  ...
  offset += 2;
  const _tokenIn = quote.swiftInputContract === import_ethers3.ZeroAddress ? nativeAddressToHexString(import_web38.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.swiftInputContract, sourceChainId);
  ...
  offset += 2;
  const _tokenOut = quote.toToken.contract === import_ethers3.ZeroAddress ? nativeAddressToHexString(import_web38.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, destinationChainId);
  ...
  offset += 8;
  const refAddress = referrerAddress ? import_buffer8.Buffer.from(hexToUint8Array(nativeAddressToHexString(referrerAddress, destinationChainId))) : import_web38.SystemProgram.programId.toBuffer();
...
function createSwiftInitInstruction(params) {
  const { quote } = params;
  const mint = quote.swiftInputContract === import_ethers3.ZeroAddress ? solMint : new import_web38.PublicKey(quote.swiftInputContract);
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  ...
  const accounts = [
    ...
    { pubkey: mint, isWritable: false, isSigner: false },
    { pubkey: new import_web38.PublicKey(addresses_default.FEE_MANAGER_PROGRAM_ID), isWritable: false, isSigner: false },
    { pubkey: new import_web38.PublicKey(addresses_default.TOKEN_PROGRAM_ID), isWritable: false, isSigner: false },
  ...
  const refAddress = params.referrerAddress ? import_buffer8.Buffer.from(hexToUint8Array(nativeAddressToHexString(params.referrerAddress, destinationChainId))) : import_web38.SystemProgram.programId.toBuffer();
  ...
  InitSwiftLayout.encode({
    ...
    destinationChain: destinationChainId,
    tokenOut: import_buffer8.Buffer.from(hexToUint8Array(nativeAddressToHexString(quote.toToken.contract, destinationChainId))),
  ...
  return new import_web38.TransactionInstruction({
    keys: accounts,
    data,
    programId: new import_web38.PublicKey(addresses_default.SWIFT_PROGRAM_ID)
  });
}
async function createSwiftFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  let swapMessageV0Params = null;
  const swiftProgram = new import_web38.PublicKey(addresses_default.SWIFT_PROGRAM_ID);
  const trader = new import_web38.PublicKey(swapperAddress);
  const randomKey = import_web38.Keypair.generate();
  if (!Number(quote.deadline64)) {
  ...
  const [state] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer8.Buffer.from("STATE_SOURCE"), hash],
    swiftProgram
  );
  const swiftInputMint = quote.swiftInputContract === import_ethers3.ZeroAddress ? solMint : new import_web38.PublicKey(quote.swiftInputContract);
  const stateAccount = getAssociatedTokenAddress(
    swiftInputMint,
    state,
    true
  );
  const relayer = quote.gasless ? new import_web38.PublicKey(quote.relayer) : trader;
...
async function createHyperCoreDepositFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const [ledger] = import_web39.PublicKey.findProgramAddressSync(
    [
      import_buffer9.Buffer.from("LEDGER_BRIDGE"),
      trader.toBytes(),
...
async function createSwapFromSolanaInstructions(quote, swapperWalletAddress, destinationAddress, referrerAddresses, connection, options = {}) {
  ...
  const [state, stateNonce] = import_web311.PublicKey.findProgramAddressSync(
    [
      import_buffer10.Buffer.from("V2STATE"),
      import_buffer10.Buffer.from(msg1.publicKey.toBytes()),
      import_buffer10.Buffer.from(msg2.publicKey.toBytes())
    ],
    mayanProgram
  );
  ...
  const destAddress = import_buffer10.Buffer.from(
    hexToUint8Array(
  ...
  const deadline = BigInt(quote.deadline64);
  const swapData = import_buffer10.Buffer.alloc(SwapLayout.span);
  const swapFields = {
    instruction: 101,
    stateNonce,
    amount: getSafeU64Blob(amount),
...
async function swapFromSolana(quote, swapperWalletAddress, destinationAddress, referrerAddresses, signTransaction, connection, extraRpcs, sendOptions, jitoOptions, instructionOptions) {
  ...
  const swapper = new import_web311.PublicKey(swapperWalletAddress);
  const feePayer = quote.gasless ? new import_web311.PublicKey(quote.relayer) : swapper;
  ...
  if (jitoEnabled) {
    ...
    if (signedTrx instanceof import_web311.Transaction && signedTrx?.signatures[0]?.publicKey) {
      mayanTxHash = import_bs582.default.encode(Uint8Array.from(signedTrx.signatures[0].signature));
    } else if (signedTrx instanceof import_web311.VersionedTransaction && signedTrx?.signatures[0]) {
  ...
  if (quote.gasless) {
    const serializedTrx = import_buffer10.Buffer.from(signedTrx.serialize()).toString("base64");
...
function getEvmSwiftParams(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId) {
  ...
  const random = nativeAddressToHexString(import_web312.Keypair.generate().publicKey.toString(), 1);
  const tokenOut = quote.toToken.contract === import_ethers6.ZeroAddress ? nativeAddressToHexString(import_web312.SystemProgram.programId.toString(), 1) : nativeAddressToHexString(quote.toToken.contract, destChainId);
  const minAmountOut = getAmountOfFractionalAmount(
    quote.minAmountOut,
    Math.min(8, quote.toToken.decimals)
  );
...
function writeBigIntTo16BytesBuffer(value) {
  ...
  const buffer = import_buffer11.Buffer.alloc(16);
  for (let i = 15; i >= 0; i--) {
...
function getShuttleParams(quote, destinationAddress, signerChainId) {
  ...
  const maxLLFeeBuffer8Bytes = import_buffer11.Buffer.alloc(8);
  ...
  const deadLineBuffer = import_buffer11.Buffer.alloc(4);
  ...
  const gasDropBuffer8Bytes = import_buffer11.Buffer.alloc(8);
  gasDropBuffer8Bytes.writeBigUInt64BE(gasDrop);
  const gasDropBytes = gasDropBuffer8Bytes.subarray(4);
  bytes.push(...gasDropBytes);
  const maxRelayerFeeBuffer8Bytes = import_buffer11.Buffer.alloc(8);
  ...
  if (quote.fromToken.contract === quote.shuttleInputContract) {
    ...
    const amountInBuffer = import_buffer11.Buffer.alloc(8);
  ...
  if (shuttleParams.hasDestSwap) {
    if (quote.toToken.contract === import_ethers7.ZeroAddress) {
      ...
      const tokenOut = import_buffer11.Buffer.from(nativeAddressToHexString(quote.toToken.contract, destChainId).slice(2), "hex");
      bytes.push(...tokenOut);
    }
    const swapDeadlineBuffer = import_buffer11.Buffer.alloc(4);
    ...
    if (quote.toChain === "solana") {
      bytes.push(...Array(8).fill(0));
      const minAmountOutBuffer = import_buffer11.Buffer.alloc(8);
    ...
    const swapPath = import_buffer11.Buffer.from(shuttleParams.path.slice(2), "hex");
    bytes.push(...swapPath);
  } else {
  ...
  return {
    destAddr: destinationAddressHex,
    destChainId,
    serializedParams: `0x${import_buffer11.Buffer.from(bytes).toString("hex")}`,
...
function getShuttleFromEvmTxPayload(quote, destinationAddress, signerChainId, permit) {
  ...
  let shuttleCallData;
  const shuttleContract = new import_ethers7.Contract(shuttleContractAddress, ShuttleArtifact_default.abi);
...
function getEvmFastMctpBridgeTxPayload(quote, destinationAddress, referrerAddress, signerChainId, payload) {
  ...
  const fastMctpContract = new import_ethers8.Contract(contractAddress, MayanFastMctpArtifact_default.abi);
  let data;
  let value;
...
function getEvmFastMctpCreateOrderParams(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const deadline = BigInt(quote.deadline64);
  const tokenOut = quote.toToken.contract === import_ethers8.ZeroAddress ? nativeAddressToHexString(import_web313.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(
    quote.toChain === "sui" ? quote.toToken.verifiedAddress : quote.toToken.contract,
    quote.toToken.wChainId
  );
  return {
...
function getUsdcDepositInitiatorMctpTxPayload(quote, swapperAddress, destinationAddress, usdcPermitSignature) {
  ...
  const initiatorContract = new import_ethers9.Contract(
    quote.hyperCoreParams.initiateContractAddress,
    HCDepositInitiatorArtifact_default.abi
  );
  const signatureBuf = import_buffer13.Buffer.from(hexToUint8Array(usdcPermitSignature));
...
function getHyperCoreDepositFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId, permit, payload, options = {}) {
  ...
  const _permit = permit || ZeroPermit;
  const forwarder = new import_ethers9.Contract(addresses_default.MAYAN_FORWARDER_CONTRACT, MayanForwarderArtifact_default.abi);
  ...
  if (quote.fromToken.contract.toLowerCase() === quote.hyperCoreParams.initiateTokenContract.toLowerCase()) {
    ...
    if (quote.fromToken.contract === import_ethers9.ZeroAddress) {
      ...
      return {
        data,
        to: addresses_default.MAYAN_FORWARDER_CONTRACT,
        value: (0, import_ethers9.toBeHex)(BigInt(quote.effectiveAmountIn64)),
        chainId: signerChainId,
        _forwarder: {
          method: forwarderMethod,
          params: forwarderParams
...
function getEvmMonoChainTxPayload(quote, destinationAddress, referrerAddress) {
  ...
  const monoChainContract = new import_ethers10.Contract(
  ...
  let data;
  let value;
  if (quote.toToken.contract === import_ethers10.ZeroAddress) {
    data = monoChainContract.interface.encodeFunctionData("transferEth", [
      destinationAddress,
      referrerAddress || import_ethers10.ZeroAddress,
      referrerBps
    ]);
  } else {
  ...
  if (quote.fromToken.contract === import_ethers10.ZeroAddress) {
    value = (0, import_ethers10.toBeHex)(quote.effectiveAmountIn64);
  } else {
    value = (0, import_ethers10.toBeHex)(0);
  }
  return {
...
function getMonoChainFromEvmTxPayload(quote, destinationAddress, referrerAddress, signerChainId, permit) {
  ...
  signerChainId = Number(signerChainId);
  const _permit = permit || ZeroPermit;
  const forwarder = new import_ethers10.Contract(
  ...
  if (quote.fromToken.contract === import_ethers10.ZeroAddress) {
    ...
    return {
      data,
      to: addresses_default.MAYAN_FORWARDER_CONTRACT,
      value: (0, import_ethers10.toBeHex)(monoChainPayloadIx._params.amountIn),
      chainId: signerChainId,
      _forwarder: {
        method: forwarderMethod,
        params: forwarderParams
...
function getEvmSwapParams(quote, destinationAddress, referrerAddress, signerAddress, signerChainId, payload) {
  ...
  const [mayanMainAccount] = import_web315.PublicKey.findProgramAddressSync(
    [import_buffer14.Buffer.from("MAIN")],
    mayanProgram
  );
  ...
  const unwrapRedeem = quote.toToken.contract === import_ethers11.ZeroAddress;
  const criteria = {
    ...
    unwrap: unwrapRedeem,
    customPayload: payload ? `0x${import_buffer14.Buffer.from(payload).toString("hex")}` : "0x"
  };
...
function getSwapFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddresses, signerAddress, signerChainId, payload, permit, options) {
  const signerWormholeChainId = getWormholeChainIdById(Number(signerChainId));
  const fromChainId = getWormholeChainIdByName(quote.fromChain);
  if (fromChainId !== signerWormholeChainId) {
    throw new Error(`Signer chain id(${Number(signerChainId)}) and quote from chain are not same! ${fromChainId} !== ${signerWormholeChainId}`);
  ...
  const forwarderContract = new import_ethers11.Contract(addresses_default.MAYAN_FORWARDER_CONTRACT, MayanForwarderArtifact_default.abi);
  const mayanSwap = new import_ethers11.Contract(contractAddress, MayanSwapArtifact_default.abi);
  let forwarderMethod;
  let forwarderParams;
  let value;
  const _permit = permit || ZeroPermit;
  if (tokenIn === import_ethers11.ZeroAddress) {
    ...
    value = (0, import_ethers11.toBeHex)(amountIn);
  } else {
    const mayanCallData = mayanSwap.interface.encodeFunctionData(
      "swap",
      [
    ...
    value = (0, import_ethers11.toBeHex)(bridgeFee);
  }
...
async function addInitOrderMoveCalls(quote, swapperAddress, destinationAddress, referrerAddress, mctpPackageId, feeManagerPackageId, suiClient, options) {
  ...
  const tokenOut = quote.toToken.contract === import_ethers12.ZeroAddress ? nativeAddressToHexString(import_web316.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, quote.toToken.wChainId);
  const amountOutMin = getAmountOfFractionalAmount(
    quote.minAmountOut,
    Math.min(8, quote.toToken.decimals)
  );
...
async function addPublishWormholeMessage(tx, messageTicket, suiClient, bridgeFee, feePayer, suiCoin) {
      ...
      const { coins, sum } = await fetchAllCoins(
        {
          ...
          coinType: import_utils20.SUI_TYPE_ARG,
  ...
  tx.moveCall({
    ...
    arguments: [
      ...
      gasCoin,
      messageTicket,
      tx.object(import_utils20.SUI_CLOCK_OBJECT_ID)
    ]
  });
  return tx;
}
